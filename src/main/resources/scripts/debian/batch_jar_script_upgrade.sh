...
#     exit 1
# fi
# Ubuntu/Debian 通过 os-release 判断，不再解析 CentOS 风格内核后缀 an8/el7
if [ -f /etc/os-release ]; then
    ...
    if [ "$ID" = "ubuntu" ]; then
        if [ -z "$ubuntu_major" ] || [ "$ubuntu_major" -lt 20 ]; then
            echo -e "${RED} error: Unsupported Ubuntu version ${VERSION_ID}. Need Ubuntu 20.04 or later${NC}"
            exit 1
        fi
    elif [ "$ID" = "debian" ]; then
        if [ -z "$ubuntu_major" ] || [ "$ubuntu_major" -lt 11 ]; then
            echo -e "${RED} error: Unsupported Debian version ${VERSION_ID}. Need Debian 11 or later${NC}"
            exit 1
        fi
    elif ! echo " ${ID_LIKE} " | grep -qw debian; then
        echo -e "${RED} error: Unsupported system ${ID}. This script is for Ubuntu/Debian${NC}"
        exit 1
    fi
else
    echo -e "${RED} error: /etc/os-release not found, cannot detect OS${NC}"
    exit 1
fi
...
if ! command -v rsync &> /dev/null  ; then
    echo -e "${YELLOW}Info: rsync is not installed, installing via apt-get...${NC}"
    if ! sudo DEBIAN_FRONTEND=noninteractive apt-get update -qq || ! sudo DEBIAN_FRONTEND=noninteractive apt-get install -y rsync >/dev/null; then
