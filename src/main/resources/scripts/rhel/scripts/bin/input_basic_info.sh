...
readDir() {
    while true; do
        ...
        if [ ! -d "$ch" ]; then
            ...
            exit 1
        elif [ -f /usr/lib/systemd/system/"${APP_NAME}".service -o -f /lib/systemd/system/"${APP_NAME}".service ]; then
...
read_APP_NAME() {
    ...
    while true; do
        ...
        if [ -f /usr/lib/systemd/system/"${ch}".service -o -f /lib/systemd/system/"${ch}".service ]; then
